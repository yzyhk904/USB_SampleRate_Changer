...
* Tips 1: You can see the sample rate connecting to a USB DAC during music replaying by a command `cat /proc/asound/card1/pcm0p/sub0/hw_params` (for non- USB hardware offload drivers). You can also see mixer ("AudioFlinger") info by a command `dumpsys media.audio_flinger`. There are corresponding convenient scripts ("alsa-hw-params.sh" and "dumpsys-filtered.sh") in "extras" folder.
  ...
  - Usage:  `sh /sdcard/USB_SampleRate_Changer/extras/change_resampling_quality.sh [--help] [--reset] [stop_band_dB [half_filter_length [cut_off_percent]]]`
    - changes the resampling quality of AudioFlinger (the OS mixer). With "--reset" option, this script clears previous settings. "stop_band_dB", "half_filter_length" and "cut off percent" specify stopband attenuation in dB, the number of input data needed before the current point (optional) and 3dB cut off percent (optional), respectively. AOSP default values are 90dB and 32 (cut off: 100% to Nyquist frequency), but this script's default values are 140dB and 320 (cut off: 91% to Nyquist frequency) as mastering quality values.
